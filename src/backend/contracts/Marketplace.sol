...
contract Marketplace is ReentrancyGuard{
    ...
    function makeItems(IERC721 _nft,uint _tokenId, uint price) external nonReentrant{
        ...
        items[itemCount] = Item(itemCount,_nft,_tokenId,price,payable(msg.sender),false);
        emit Offered(itemCount, address(_nft), _tokenId, price, msg.sender);
    }

    function purchaseItems(uint _itemId) external payable nonReentrant{
        uint _totalPrice =  getTotalPrice(_itemId);
        Item storage item = items[_itemId];
        require(_itemId > 0 && _itemId <= itemCount,"item doesn'ty exist");
